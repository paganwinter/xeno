...
class Router {
  ...
  on(method = 'GET', path, data) {
    ...
    const routeData = { ...data, method, path };
    ...
    methods.forEach((mth) => {
      ...
      this.trees[`${mth}`].addRoute(path, routeData);
    });
  ...
  find(method, path) {
    ...
    if (methodTree) {
      ...
      if (pathNode.handle !== null) {
        ...
        const { handle: routeData, params: paramsArr } = pathNode;
        const params = paramsArr.reduce((acc, item) => {
          acc[`${item.key}`] = item.value;
          return acc;
        }, {});
        debug('route found', routeData.method, routeData.path);
        return { route: routeData, params };
      }
    }
